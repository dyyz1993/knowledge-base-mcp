...
export default function App() {
  ...
  const [tab, setTab] = useState<Tab>(() => {
    ...
    } catch (e) { if (import.meta.env.DEV) console.warn('[App] localStorage.getItem failed:', e) }
    return "kb"
  })
  const handleSetTab = (t: Tab) => {
    setTab(t)
    try { localStorage.setItem("kb-active-tab", t) } catch (e) { if (import.meta.env.DEV) console.warn('[App] localStorage.setItem failed:', e) }
  }
  ...
  useEffect(() => {
    (async () => {
      ...
      } catch (e) { if (import.meta.env.DEV) console.warn('[App] health check failed:', e) }
    })()
  }, [])
